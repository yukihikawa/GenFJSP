...
#这个类是一个Q-Network，它是一种用于强化学习的神经网络模型。
# Q-Network用于估计状态-动作对的Q值，这些Q值表示在给定状态和动作之后所有可能的未来奖励的期望值。

#它使用一个称为"build_mlp"的函数来构建其内部网络，该网络是一个多层感知机(MLP)，由输入层，一个或多个隐藏层和输出层组成。

#在init函数中，它接受三个参数："dims"是一个列表，表示隐藏层的维度，"state_dim"是输入层的维度，"action_dim"是输出层的维度。
# 在创建网络之后，它还存储了一个explore_rate属性，用于在探索和探索之间进行平衡。

#该类还定义了一个forward函数，该函数接受一个"state"张量作为输入，并返回一个张量，其中包含网络对于多个动作的Q值。
# 此外，还定义了一个get_action函数，该函数接受一个"state"张量作为输入，并返回一个整数张量，其中包含网络选择的动作的索引。
# 在选择动作时，该函数可能使用贪心策略，即选择Q值最大的动作，也可能使用随机策略，即随机选择
...
#该函数的作用是构建一个多层感知机(MLP)。
# 它接受一个参数"dims"，该参数是一个包含整数的列表，表示每层的维度。
# 函数使用循环来遍历每一层，并在每一层之间添加一个线性层(nn.Linear)和一个ReLU激活函数(nn.ReLU)。
# 最后，函数删除最后一个激活函数，因为输出层通常不需要激活函数。
# 最后返回一个Sequential容器，包含所有的线性层和激活函数。
...
#该函数的作用是过滤一个函数的关键字参数。
# 它接受两个参数："function"是要过滤的函数，"kwargs"是要过滤的关键字参数字典。
# 函数使用Python的inspect模块获取函数的签名，并将其参数名称存储在一个集合中。
# 然后，它使用集合操作计算kwargs字典中的键和函数签名中的参数名称的交集。
# 最后，它返回一个新字典，其中包含kwargs字典中与函数签名中的参数名称相匹配的键和值。
...
#这个函数的作用是构建一个环境。它接受两个参数："env_class"是环境的类，"env_args"是一个字典，包含创建环境所需的参数。
#首先，函数使用"gym.envs.registration"模块中的类来判断是否是gym环境。如果是gym环境，它会使用gym包中的版本来构建环境，并检查版本是否在预定范围内。
#否则，它会使用kwargs_filter函数过滤env_args字典中的参数，仅保留与环境类的构造函数中的参数名称相匹配的参数。然后使用过滤后的参数创建环境。
#最后，函数还会将一些额外的信息，例如环境名称、状态维度、动作维度和动作类型（离散/连续）存储在环境对象中，并返回该环境对象。
...
class AgentBase:
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        self.repeat_times = args.repeat_times # repeat times for training
        self.reward_scale = args.reward_scale # reward scale
...
class AgentDQN(AgentBase):
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        self.cri_class = getattr(self, "cri_class", None)  # means `self.cri = self.act` 意思是
